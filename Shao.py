...
class MainSHare:
    def __init__(self):
        ...
        try:
            self.open_file = open(input(self.format_print("*", "INP TOKEN FILE: "))).read().split('\n')
            self.open_file.remove('')
            self.total = str(len(self.open_file))
        except:
            quit(self.format_print("$", 'No Such File!!!'))
    ...
    def banner(self):
        ...
        os.system("title Share-Bot FaceBook by @HuyKazuto" if os.name == "nt" else "")
        title = '\n\n\n--RCode by HuyKazuto--'
        banner = '''\n \n 		
  ░█─░█ █──█ █──█ ░█─▄▀ █▀▀█ ▀▀█ █──█ ▀▀█▀▀ █▀▀█ 
		░█▀▀█ █──█ █▄▄█ ░█▀▄─ █▄▄█ ▄▀─ █──█ ──█── █──█ 
    ...
    def run_share(self):
        while True:
            main.banner()
            try: 
                id_post = input(self.format_input("!",f"INPUT POST ID: "))
